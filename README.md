...
источники. Неизвестный VIN бесплатно декодируется через NHTSA vPIC и сохраняется
в очередь на ручную проверку. Непроверенные номера турбин пользователю не
выдаются.
