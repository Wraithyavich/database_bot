...
Для одного пользователя можно настроить сообщение перед его первым VIN-запросом
за календарный день по московскому времени:

```dotenv
VIN_DAILY_GREETING_USER_ID=123456789
VIN_DAILY_GREETING_TEXT=Персональное сообщение
```
...
модельному префиксу, после чего YandexGPT структурирует найденные сниппеты.
Каждый возвращённый OEM/Turbo P/N дополнительно отбрасывается, если его нет
дословно ни в одном поисковом источнике. Резервный провайдер — Gemini с Google
Search grounding. Возможные номера сопоставляются точным поиском с основной
SQLite-базой, а ответ сохраняется в кэш и очередь ручной проверки. Такой
...
выполняется не более одного онлайн-поиска. Повторный VIN берётся из SQLite-кэша
без нового платного обращения.
